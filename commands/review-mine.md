...
- **if the diff touches `expel-design-system` (EDS) or styles** (component/CSS changes in `expel-design-system` itself, or a consumer repo's `.tsx`/`.module.css`/`.scss` files that use EDS components or `var(--eds-*)` tokens): instruct the agent to run the EDS audit tooling as real verification, not just a vibe check — `npx expel-design-system eds-audit --path .` in the consumer repo (clone/checkout the branch if needed) to get real component/token counts, and check the diff against `expel-design-system`'s own anti-patterns reference (hardcoded hex/px instead of tokens, `className` carrying Bootstrap-style utility classes instead of EDS's own spacing/color system, standalone typography vars instead of `<Text>`/`<Title>`). If the consumer repo's `.eds-context.md` is stale (version mismatch or missing), flag it as its own finding — don't silently work around it. Skip this for PRs with no EDS/styling surface.
